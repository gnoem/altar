import { IRawKeyframeMap } from "@types";
import * as THREE from "three";

const getKeyframeTracks = (states: string[], times: number[], rawKeyframeData: () => IRawKeyframeMap) => {
...
const animate = (rawKeyframeData: () => IRawKeyframeMap) => {
  return (
    ...
    const clip = new THREE.AnimationClip(states.join('-to-'), -1, getKeyframeTracks(states, times, rawKeyframeData));
    play(clip);
  }
}

const exposeAnimationData = (rawKeyframeData: () => IRawKeyframeMap) => {
  return {
    rawKeyframeData,
    playAnimation: animate(rawKeyframeData)
  }
}
